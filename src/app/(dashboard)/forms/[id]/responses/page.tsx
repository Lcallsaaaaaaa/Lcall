...
import { linkFormResponseToFriend } from "@/features/forms/actions";
import { getFormResponses } from "@/features/forms/queries";
...
export default async function FormResponsesPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-[1200px] p-6 lg:p-8">
      <Link href={`/forms/${id}`} className="mb-4 inline-flex items-center gap-1 text-sm text-muted transition hover:text-ink">
      ...
      <div className="mb-6 flex flex-wrap items-end justify-between gap-3">
        ...
        <a href={`/api/export/form-responses?formId=${id}`} className={buttonClasses("outline", "md")}>
            ...
            <table className="w-full border-collapse text-sm">
              <thead>
                <tr className="border-b border-line bg-surface-2">
                  ...
                  {fields.map((f) => (
                    <th key={f.id} className="px-5 py-2.5 text-left text-xs font-medium uppercase tracking-wide text-muted">
                      {f.label}
                    </th>
                  ))}
                </tr>
                  ...
                  <tr key={r.id} className="border-b border-line last:border-0 hover:bg-surface-2/60">
                    ...
                    <td className="px-5 py-3 text-ink">
                      {r.friendId ? (
                        ...
                        <form action={linkFormResponseToFriend.bind(null, r.id, id)} className="flex items-center gap-1">
                    ...
                    {fields.map((f) => (
                      <td key={f.id} className="px-5 py-3 text-ink">
                        {r.values[f.id] ?? "—"}
                      </td>
                    ))}
                  </tr>
